...
-- SELECT 
-- 	CONCAT(quantity, ' (', yr, ')') "P4 product: Quantity Sold"
-- FROM (
-- 	SELECT
-- 		year(date) yr,
...
-- 		orders
-- 	WHERE 
-- 		productid = 'P4' and year(date) in (2020, 2021, 2022)
-- 	GROUP BY 	
-- 		year(date)
-- ) as t;
...
-- DELIMITER //
-- CREATE FUNCTION FindSoldQuantity(productidinput char(2), yrinput int) RETURNS DECIMAL(6,2) DETERMINISTIC 
-- BEGIN
-- RETURN (SELECT sum(quantity) FROM orders WHERE productid = productidinput and year(date) = yrinput);
-- END//
-- DELIMITER ;
